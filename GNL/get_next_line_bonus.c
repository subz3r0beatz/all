...
/*   Updated: 2025/11/15 17:05:03 by fldumas-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	buffer[fd] = ft_protectandmalloc(fd, buffer[fd]);
	if (!buffer[fd])
		return (NULL);
	line = NULL;
	readsize = ft_protectedread(buffer[fd], line, fd, 0);
	while (readsize > 0)
	{
		line = ft_get_line(&buffer[fd], line, readsize);
		if (!line)
			return (NULL);
		if (ft_strchr(line, '\n'))
			return (line);
		readsize = ft_protectedread(buffer[fd], line, fd, 1);
		if (readsize == -1)
			return (NULL);
		if (!readsize)
			break ;
